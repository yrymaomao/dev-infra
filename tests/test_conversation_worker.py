...
from ebiz_deployment.supply_chain_bff.conversation_worker import ConversationWorker
...
async def test_adapter_denial_terminates_turn_without_retry_or_business_claim(status):
    ...
    settings = SimpleNamespace(openclaw_ingress_url="http://127.0.0.1/ingress")
    requests = []
    ...
    worker = ConversationWorker(repository, settings)
